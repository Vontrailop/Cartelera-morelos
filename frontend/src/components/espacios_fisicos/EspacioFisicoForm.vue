<template>
  <div class="bg-white p-6 rounded-lg shadow-md">
    <h2 class="text-xl font-bold mb-6">{{ isEditing ? 'Editar' : 'Nuevo' }} Espacio Físico</h2>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Dimensiones (m²)</label>
          <input 
            v-model.number="form.dimensiones" 
            type="number" 
            min="1"
            step="0.01"
            class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
      ...
      <div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
          <div v-for="i in 3" :key="i" class="relative bg-gray-100 rounded-lg overflow-hidden h-32">
